import numpy as np, codecs, json, pickle, sys, random, itertools
...
# def do_stuff_n_things(corpus, num_topics, attr):
...
for doc in corpus.documents:
    ...
    cl_doc = [x for x in cl_doc if x]

    cl_docs.append(cl_doc)

model = lda_gibbs_sampling_copula(K=num_topics, docs=cl_docs, V=len(corpus.vocabulary))
for _ in range(1):
    model.inference()

for doc, z_n in zip(corpus.documents, model.z_m_n):
    doc.metadata[attr] = sum(z_n.tolist(), [])
